 ...
 * @version 1.1
 ...
 var events = {
        ...
        grid:{
            ...
            operationEvents:[{name:"删除",class:"",icon:"",click:function(rows){
                    var rows = $scope.lionFormGrid.getSelectedRows();
                    //TODO
 ...
 $scope.lionFormGrid = {options:options,events:events}
 ...
 5.1
 //grid数据加载完成后的事件
 $scope.lionFormGrid.gridLoaded(rows) rows为当前页所有数据
 5.2
 //重新以当前参数查询第一页数据
 $scope.lionFormGrid.reLoadData();
 5.3
 //当showClose为true时有效
 $scope.lionFormGrid.modalClose();

 5.3
 //提供获取所有被选中的数据
 $scope.lionFormGrid.getAllSelectRows();
...
lionFormGridDirectives.directive('lionFormGrid', ['dbUtils', function (dbUtils) {
    return {
        ...
        controller: ['$scope', 'dbImService', function ($scope, dbImService) {
            ...
            if (angular.isUndefined($scope.lionFormGrid)) {
                return;
            }

            $scope.lionFormGrid.queryParams = $scope.lionFormGrid.options.form.hiddenParams || {};

            $scope.lionFormGrid.sortingParams = null ;

            var selectFields = [];
            //计算每个占用的列数
            var fieldCols = 12 / $scope.lionFormGrid.options.form.settings.cols;
            angular.forEach($scope.lionFormGrid.options.form.fields, function (field) {
                ...
                $scope.lionFormGrid.queryParams[field.name] = null;
                if (!angular.isArray(field.dropDownItem) && field.dropDownItem && field.type == "select") {
                    $scope.lionFormGrid.queryParams[field.name] = {"name": "", "value": null};
            ...
            var autoLoad = $scope.lionFormGrid.options.grid.settings.autoLoad;
            autoLoad = angular.isUndefined(autoLoad) ? true : autoLoad;
            if (!autoLoad) {
                $scope.lionFormGrid.loadingTip = "大饼君，正在候驾中...";
            } else {
                queryData();
            }
            ...
            //ajax异步获取数据
            function queryData() {
                $scope.lionFormGrid.loadingTip = "大饼君，正在努力为您加载数据,请稍候...";
                var queryParams = angular.copy($scope.lionFormGrid.queryParams);
                angular.forEach(selectFields, function (selectField) {
                    queryParams[selectField] = $scope.lionFormGrid.queryParams[selectField].value;
                });
                queryParams["page"] = {
                ...
                dbUtils.post($scope.lionFormGrid.options.grid.settings.transCode, queryParams, function (data) {
                    ...
                    for (var i in rows) {
                        ...
                        for (var j in $scope.lionFormGrid.options.grid.header) {
                            ...
                            if (!angular.isUndefined($scope.lionFormGrid.events.grid.fieldEvents)) {
                                ...
                                var clickEvent = $scope.lionFormGrid.events.grid.fieldEvents[header.field + 'Click'];
                                clickEvent ? header.clickEvent = clickEvent : null;

                                var formatEvent = $scope.lionFormGrid.events.grid.fieldEvents[header.field + 'Format'];
                    ...
                    $scope.lionFormGrid.page = pages;
                    if (angular.isFunction($scope.lionFormGrid.gridLoaded)) {
                        $scope.lionFormGrid.gridLoaded($scope.lionFormGrid.rows);
                    }
                    Metronic.stopPageLoading();
                    checkAllowSelect();
                    $scope.lionFormGrid.loadingTip = "大饼君已努力，但并无数据...";
                }, function () {
                    $scope.lionFormGrid.loadingTip = "大饼君貌似生病啦，请稍等，再重试...";
                });
            }

            $scope.lionFormGrid.reLoadData = function () {
                $scope.lionFormGrid.page.pageNumber = 1;
                queryData();
            };

            //分页数量点击事件
            $scope.lionFormGrid.pageNumberClick = function (pageNumber) {
                var prevPage = $scope.lionFormGrid.page.prevPageDisabled;
                if (pageNumber === "prev" && prevPage && prevPage != "") {
                    return false;
                }
                var nextPage = $scope.lionFormGrid.page.nextPageDisabled;
                if (pageNumber === "next" && nextPage && nextPage != "") {
                    return false;
                }
                if (pageNumber == $scope.lionFormGrid.page.pageNumber) {
                    return false;
                }
                if (pageNumber === "...") {
                    return false;
                ...
                if (pageNumber === "prev") {
                    $scope.lionFormGrid.page.pageNumber--;
                } else if (pageNumber === "next") {
                    $scope.lionFormGrid.page.pageNumber++;
                } else {
                    $scope.lionFormGrid.page.pageNumber = pageNumber;
                }
                queryData();
            };
            //grid上方按钮点击事件
            $scope.lionFormGrid.operationButtonClick = function (clickFun) {
                //获取所有已经选择的行数据传递给回调方法。
                var rows = getAllSelectRows();
                clickFun(rows, $scope.lionFormGrid.rows);
                checkAllowSelect();
            };

            //每行点击事件
            $scope.lionFormGrid.rowClick = function (row) {
                row.checked = !row.checked;
                checkAllowSelect();
            };

            //点击全选复选框事件
            $scope.lionFormGrid.allRowClick = function () {
                $scope.lionFormGrid.options.grid.settings.allRowChecked = !$scope.lionFormGrid.options.grid.settings.allRowChecked;
                angular.forEach($scope.lionFormGrid.rows, function (row) {
                    row.checked = $scope.lionFormGrid.options.grid.settings.allRowChecked;
                });
            };

            function checkAllowSelect() {
                if (!$scope.lionFormGrid.options.grid.settings.showCheckBox) {
                    return;
                }
                //如果所有行数据为非选中状态，则全选按钮为非选中状态，反之一样
                var flag = true;
                if ($scope.lionFormGrid.rows.length == 0) {
                    flag = false;
                }
                angular.forEach($scope.lionFormGrid.rows, function (row) {
                    if (!row.checked) {
                ...
                $scope.lionFormGrid.options.grid.settings.allRowChecked = flag;
            }
